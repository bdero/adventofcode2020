input = """LLLLLLLLLL.LLLL.LLLLLLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLL.LL.LLLLLLLLLLL.LLLLLL.LL.LLLL.LLLLLL.LLLLLLL
...
def runrule(gameboard, crowded, adjacent):
    dirty = False
    occupied = 0
    newboard = [["."]*w for y in range(h)]
    for y, row in enumerate(gameboard):
        for x, value in enumerate(row):
            ...
            if value == "L":
                switch = True
                for j, k in adjacent(x, y, gameboard):
                    if gameboard[k][j] == "#":
                        switch = False
                        break
                newboard[y][x] = "#" if switch else "L"
            else: # Value == #
                n = 0
                for j, k in adjacent(x, y, gameboard):
                    if gameboard[k][j] == "#":
                        n += 1
                newboard[y][x] = "L" if n >= crowded else "#"
            if newboard[y][x] != gameboard[y][x]:
                dirty = True
            if newboard[y][x] == "#":
                occupied += 1
    return dirty, newboard, occupied

dirty = True
resultboard = lines
occupied = 0
while dirty:
    dirty, resultboard, occupied = runrule(resultboard, 4, adj)
...
def cross(x, y, g):
    for xx, yy in slopes:
        d = 1
        while(0 <= x + xx*d < w and 0 <= y + yy*d < h):
            nx, ny = x + xx*d, y + yy*d
            yield nx, ny
            if g[ny][nx] == "#" or g[ny][nx] == "L":
                break
            d += 1
...
occupied = 0
while dirty:
    dirty, resultboard, occupied = runrule(resultboard, 5, cross)
print(occupied)