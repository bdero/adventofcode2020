...
for passport in passports:
    ...
    for field, value in passport.items():
        ...
        if field == "byr": valid = valid and inrange(value, 1920, 2002)
        elif field == "iyr": valid = valid and inrange(value, 2010, 2020)
        elif field == "eyr": valid = valid and inrange(value, 2020, 2030)
        elif field == "hgt":
            unit = value[-2:]
            if unit == "cm":
                try:
                    ...
                    valid = False
                valid = valid and 150 <= n <= 193
            elif unit == "in":
                try:
                    n = int(value[:2])
                except:
                    valid = False
                valid = valid and 59 <= n <= 76
            else:
                valid = False
        elif field == "hcl":
            if not re.fullmatch("#[a-f0-9]{6}", value):
                valid = False
        elif field == "ecl":
