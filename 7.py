input = """muted white bags contain 4 dark orange bags, 3 bright white bags.
...
for line in lines:
    tokens = re.split(", | contains? ", line)
    tokens = [token.split(" ") for token in tokens]
    # dark white bags
    container = f"{tokens[0][0]} {tokens[0][1]}"
    for token in tokens[1:]:
        # 4 dull aqua bags.
        within[f"{token[1]} {token[2]}"].append(container)

bags = deque(["shiny gold"])
seen = set()
while len(bags):
    bag = bags.popleft()
    if bag in seen:
...
for line in lines:
    ...
    if tokens[1][0] == "no":
        continue
    contains[container] = [(int(t[0]), f"{t[1]} {t[2]}") for t in tokens[1:]]

def dft(num, token):
    children = contains[token]
    if not children:
        return num
    return num + num*sum(dft(child[0], child[1]) for child in children)
print(dft(1, "shiny gold") - 1)
